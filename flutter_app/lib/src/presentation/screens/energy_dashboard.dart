...
class _EnergyDashboardState extends ConsumerState<EnergyDashboard>
  ...
  Future<void> _downloadLogFile(String logType) async {
    try {
      ...
      if (kIsWeb) {
        final response =
          await http.get(buildApiUri('logs/download/$logType'), headers: getApiHeaders());
    ...
    try {
      // NOTE: server expects `appliance_id` (snake_case)
        final currentResponse = await http
          .get(buildApiUri('energy', {'appliance_id': '$selectedApplianceId'}), headers: getApiHeaders())
          .timeout(const Duration(seconds: 10));

        final historyResponse = await http
          .get(buildApiUri('energy/history', {'appliance_id': '$selectedApplianceId'}), headers: getApiHeaders())
          .timeout(const Duration(seconds: 10));
  ...
  Future<void> _fetchHistoricalData({int days = 7}) async {
    ...
    try {
      final response = await http
          .get(buildApiUri('logs/historical-data', {'days': '$days'}), headers: getApiHeaders())
  ...
  Future<void> _fetchAppliances() async {
    try {
      final response = await http.get(buildApiUri('appliances'), headers: getApiHeaders());
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (mounted) {
          setState(() {
